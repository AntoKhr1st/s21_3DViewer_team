...
/// @brief  Считает количество вершин и поверхностей; 0 = error
int count_VF(obj_data *Data, FILE *file) {
  int error;
  char str_file[buff_size];

  while (fgets(str_file, sizeof(str_file), file)) {
    if (check_first_char(str_file, 'v')) {
      Data->num_V++;
    } else if (check_first_char(str_file, 'f')) {
      Data->num_F++;
    }
  }
  error = Data->num_V && Data->num_F ? OK : FILE_STRUCT_ERR;
  rewind(file);
...
/// @brief  ADD координаты вершин и точки на поверхностях
...
/// @brief  ADD координаты вершин в одной строке в Data
...
/// @brief Счетчик точек в строке f и правильность первых >=3 точек, <= num_V
...
/// @brief ADD точки на поверхности в одной строке в Data; Ошибка точки > num_V
...
/// @brief  перемещение всех точек в новую систему координат со сдвинутым центром
